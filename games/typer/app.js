...
function bindUI(){
  ...
  el("btnJoinRoom").onclick = async ()=>{
    ...
    if(code.length < 6 || code.length > 7){ showToast(getLang()==="en" ? "Code must be 6-7 chars" : "Kod musi mieć 6-7 znaków"); return; }
    await joinRoom(code);
  };
...
async function openRoom(code, opts={}){
  ...
  code = (code||"").trim().toUpperCase();
  if(!code || code.length < 6 || code.length > 7) throw new Error("Bad code");
...
async function startFromGameRoom(ctx){
  ...
  localStorage.setItem(KEY_LAST_PLAYERNO, ctx.playerNo || "");
  localStorage.setItem(KEY_ACTIVE_ROOM, ctx.room || "");
  try{ __setAuthedThisSession(ctx.playerNo); }catch(e){}
  const prof = getProfile() || {};
  prof.nick = ctx.nick;
  prof.playerNo = ctx.playerNo;
  prof.fromGameRoom = true;
  prof.updatedAt = Date.now();
  setProfile(prof);

  const ref = roomRef(ctx.room);
  const snap = await boot.getDoc(ref);
  if(!snap.exists()){
    await boot.setDoc(ref, {
      name: ctx.roomName || ctx.room,
      adminUid: userUid,
      adminNick: ctx.nick,
      createdAt: boot.serverTimestamp(),
      currentRoundNo: 1,
    ...
    await boot.setDoc(ref, { name: ctx.roomName || snap.data()?.name || ctx.room, gameRoomAdminPlayerNo: ctx.playerNo, updatedAt: boot.serverTimestamp() }, { merge:true });
  }

  await boot.setDoc(boot.doc(db, "rooms", ctx.room, "players", userUid), {
    nick: ctx.nick,
    uid: userUid,
    playerNo: ctx.playerNo,
    joinedAt: boot.serverTimestamp(),
    lastActiveAt: boot.serverTimestamp(),
    fromGameRoom: true
  }, { merge:true });

  showCenterLoading(getLang()==="en" ? "OPENING TYPER......" : "OTWIERAM TYPERA......");
  await openRoom(ctx.room, { force:true });
  hideCenterLoading();
}
...
(async()=>{
  try{
    ...
    const gameRoomContext = getGameRoomContext();
    if(gameRoomContext){
      await startFromGameRoom(gameRoomContext);
      return;
    }
