...
async function hardUpdateGameRoom(){
  try{
    const KEY='game_room_seen_version';
    ...
    if(seen!==VERSION){
      ...
      if(url.searchParams.get('v')!==VERSION.replace(/\D/g,'')){
        url.searchParams.set('v', VERSION.replace(/\D/g,''));
  ...
  try{
    ...
    if(window.caches){
      caches.keys().then(keys=>{
        keys.filter(k=>/game|room|gr|cache|pwa/i.test(k)).forEach(k=>caches.delete(k).catch(()=>{}));
      }).catch(()=>{});
    }
  }catch(e){}
}
  ...
  app.innerHTML=`<section class="screen games games-${l}">
    ...
    <div class="games-graphic-panel">
      ...
      <div class="games-grid">
        ${games.map(g=>`<button class="game-graphic-btn" data-game="${g.id}" aria-label="${l==='en'?g.en:g.pl}"><img src="assets/buttons/games/${dir}/${g.img}.png?v=1068" alt="${l==='en'?g.en:g.pl}"></button>`).join('')}
      </div>
      <button id="gamesBackBtn" class="game-graphic-back" aria-label="${l==='en'?'Back':'Cofnij'}"><img src="assets/buttons/games/${dir}/back.png?v=1068" alt="${l==='en'?'Back':'Cofnij'}"></button>
    </div>
    ${version()}
  </section>`;
